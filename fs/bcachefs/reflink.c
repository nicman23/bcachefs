		     ...
		     u64 remap_sectors, u64 new_i_size)
{
	...
	src_iter = __bch2_trans_get_iter(&trans, BTREE_ID_EXTENTS, src_start,
					 BTREE_ITER_SLOTS|BTREE_ITER_INTENT, 1);
	dst_iter = __bch2_trans_get_iter(&trans, BTREE_ID_EXTENTS, dst_start,
					 BTREE_ITER_INTENT, 2);

	while (bkey_cmp(dst_iter->pos, dst_end) < 0) {
		bch2_trans_begin_updates(&trans);
		trans.mem_top = 0;

		bch2_btree_iter_set_pos(src_iter,
			POS(src_start.inode,
			    src_start.offset +
			    (dst_iter->pos.offset - dst_start.offset)));

		src_k = bch2_btree_iter_peek_slot(src_iter);
		ret = bkey_err(src_k);
		if (ret)
			goto btree_err;
...
err:
	wrote = dst_iter->pos.offset - dst_start.offset;
	BUG_ON(wrote > remap_sectors);
